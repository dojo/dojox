define("dojox/math/curves", ["dojo", "dojox/math/_base"], function (dojo, dm) {
	var curves = {
			...
			this.getValue = function (step) {
				...
				for (var j = 0; j < this.p[0].length; j++) {
					...
					for (var i = 0; i < this.p.length; i++) {
						C += this.p[i][j] * this.p[this.p.length - 1][0] * dm.bernstein(step, this.p.length, i);
					}
					for (var l = 0; l < this.p.length; l++) {
						D += this.p[this.p.length - 1][0] * dm.bernstein(step, this.p.length, l);
					}
					retVal[j] = C / D;
				}
				return retVal;
		...
		Arc:function (start, end, ccw){
			...
			var center = dm.midpoint(start, end);
			var sides = translate(invert(center), start);
			var rad = Math.sqrt(Math.pow(sides[0], 2) + Math.pow(sides[1], 2));
			var theta = dm.radiansToDegrees(Math.atan(sides[1] / sides[0]));
			if (sides[0] < 0){
				theta -= 90;
			} else {
				theta += 90;
			}
			dm.curves.CenteredArc.call(this, center, rad, theta, theta + (ccw ? -180 : 180));
		},
		CenteredArc:function (center, radius, start, end) {
			...
			this.getValue = function (n) {
				...
				var theta = dm.degreesToRadians(this.start + ((this.end - this.start) * n));
		...
		Circle:function(center, radius){
			dm.curves.CenteredArc.call(this, center, radius, 0, 360);
			return this;
		},
		Path:function () {
			var curves = [];
			...
			this.getValue = function (n) {
				...
				for (var j = 0; j < i; j++) {
					value = dm.points.translate(value, curves[j].getValue(1));
				}
				return value;
			};
