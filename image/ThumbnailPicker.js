...
dojo.declare("dojox.image.ThumbnailPicker",
	...
	init: function(){
		if(this.isInitialized || this.maxPhotos < 1) {return;}
		...
		if(!this.thumbCells){
			...
			for(var i = 0; i < this.numberThumbs + 2; i++){
				...
				if(i > 0 && i < this.numberThumbs + 1){
					...
					if(i == 0){
						dojo.addClass(cell, "thumbPrev");
					}else{
						dojo.addClass(cell, "thumbNext");      
					}
				}
	...
	_addData: function(items){
			...
			for(var i = 0; i < _this.numberThumbs; i++){
				if(imagesToPlace[i]) {
					...
					if(node){
						...
						_this.loadNextThumbnail(loadPos);
					}
				}
			}
			...
			if(_this.maxItems < 0 && idx + _this.pageSize > _this.imageData.length){
				...
				var complete = function(items, request){
					if(items && items.length) {
						console.log("ThumbPicker: complete", [items, request]);
						_this._addData(items);
  	...
  	updateNavControls: function(){
		...
		if(this.thumbIndex < 1){
			dojo.removeClass(firstCell,"thumbPrev");
		}else if(!dojo.hasClass(firstCell,"thumbPrev")){
			dojo.addClass(firstCell,"thumbPrev");
		}
		...
		if(this.thumbIndex + this.numberThumbs >= this.maxPhotos){
			dojo.removeClass(lastCell,"thumbNext");
		} else if(!dojo.hasClass(lastCell,"thumbNext")){
			dojo.addClass(lastCell,"thumbNext");
		}
  	},
