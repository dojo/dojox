...
], function(declare, _Plugin, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Dialog, Menu, MenuItem, MenuSeparator, insertTableTemplate, modifyTableTemplate, tableDialogStrings){
	...
	var TableHandler = declare(_Plugin, {
		...
		getTableInfo: function(forceNewData){
			...
			if(this._savedTableInfo){
				// Don't query the selection when the iframe is blurred; doesn't work on IE.
			...
			if(tbl){
				...
				// Means there's no table in focus, because o.tbl is undefined.
				o = {};
			}
		...
		checkAvailable: function(){
			...
			// And use _savedTableInfo to avoid querying the selection when the iframe is blurred (doesn't work on IE)
