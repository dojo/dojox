...
(function(){
	...
	dxc.Theme = function(/*Object?*/ kwArgs){
		...
		this.colors  = (kwArgs.colors || def.colors).slice(0);
		this.sThemes = kwArgs.sThemes && kwArgs.sThemes.length ? kwArgs.sThemes.slice(0) : null;
		...
		this._current = {color: 0, marker: 0, series: 0};
		this._markers = [];
	...
	dxc.Theme._def = {
		...
		sThemes: null,	// mini-themes for individual series
	...
	dojo.extend(dxc.Theme, {
		...
		_clone: function(){
			...
			return new dxc.Theme({
				...
				sThemes: this.sThemes,
				antiAlias: this.antiAlias,
				assignColors: this.assignColors,
				assignMarkers: this.assignMarkers
			});
		},
		...
		useColors: function(){
			//	summary:
			//		returns a flag if this theme uses variable colors for different
			//		series of data (if true), or different series descriptors (if false)
			return !this.sThemes;	// Boolean
		},
		...
		next: function(/*String?*/ type){
			...
			switch(type){
				...
				case "series":
					return this.sThemes ? dojo.delegate(this.series, this.sThemes[this._current.series++ % this.sThemes.length]) : this.series;
				case "color":
				default:
