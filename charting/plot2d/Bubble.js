	...
	dojo.declare("dojox.charting.plot2d.Bubble", dojox.charting.plot2d.Base, {
		...
		render: function(dim, offsets){
			...
			var t = this.chart.theme, useColors = t.useColors(),
				sTheme, stroke, fill, outline, color, shadowStroke, shadowColor,
			...
			for(var i = this.series.length - 1; i >= 0; --i){
				...
				run.dyn.fill   = dojo.clone(fill);
				run.dyn.stroke = dojo.clone(stroke);
				...
				frontCircles = dojo.map(points, function(item){
					...
					return s.createCircle({cx: item.x, cy: item.y, r: item.radius}).setStroke(stroke).setFill(this._shapeFill(fill, rect));
				}, this);
