define(["dojo/_base/lang", "dojo/_base/array" ,"dojo/_base/declare", "dojo/dom-geometry", "dojo/_base/Color", "dojo/fx/easing",
		"./Base", "./_PlotEvents", "./common",
		"dojox/gfx", "dojox/gfx/fx", "dojox/gfx/matrix", "dojox/lang/functional", "dojox/lang/utils","dojo/has"],
	function(lang, arr, declare, domGeom, Color, easing, Base, PlotEvents, dc, g, gfxFx, m, df, du, has){
	...
	var TRANSLATE_SHIFT = 10; // Used to do a selection slide out.
		...
		render: function(dim, offsets){
			...
			arr.some(slices, function(slice, i){
				...
				if(theme.series.fill && theme.series.fill.type === "radial" && this.opt.radGrad === "fan" && step > fanSize){
					...
					var j;
					for(j = 0; j < nfans; ++j){
						var	alpha = localStart + (j - FUDGE_FACTOR) * delta,
				...
				if(this.opt.labelStyle == "default"){
					...
					arr.forEach(labeledSlices, function(slice){
						...
						if(events && !this.opt.htmlLabels){
							...
							this._connectEvents(o);
							eventSeries[slices.length + slicce.index] = o;
						}
					}, this);
				}
			}
