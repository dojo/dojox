...
(function(){
	...
	dojo.declare("dojox.charting.plot2d.Pie", dojox.charting.Element, {
		...
		render: function(dim, offsets){
			...
			this.dirty = false;
			this.cleanGroup();
			var s = this.group, t = this.chart.theme, useColors = t.useColors();
			this.resetEvents();
			...
			dojo.some(slices, function(slice, i){
				...
				if(slice >= 1){
					...
					if(typeof v == "object"){
						...
						stroke = v.stroke ? v.stroke : (useColors ? dc.augmentStroke(t.series.stroke, color) : sTheme.stroke);
						fill = v.fill ? v.fill : (useColors ? dc.augmentFill(t.series.fill, color) : sTheme.fill);
					}else{
						if(useColors){
							// need autogenerated color
							color = new dojo.Color(t.next("color"));
						...
						stroke = useColors ? dc.augmentStroke(t.series.stroke, color) : sTheme.stroke;
						fill = useColors ? dc.augmentFill(t.series.fill, color) : sTheme.fill;
					}
				...
				// draw the slice
				var color, fill, stroke, sTheme, shape;
				if(typeof v == "object"){
					if(useColors){
						if(!v.fill || !v.stroke){
							// need autogenerated color
				...
				if(fill && fill.type === "radial" && this.opt.radGrad === "fan" && step > fanSize){
					...
					for(var j = 0; j < nfans; ++j){
						var fansx = j == 0 ? x1 : circle.cx + r * Math.cos(start + (j - FUDGE_FACTOR) * delta),
							...
							fan = group.createPath({}).
								...
								closePath().
								setFill(this._pseudoRadialFill(fill, {x: circle.cx, y: circle.cy}, r, start + (j + 0.5) * delta, start + (j + 0.5) * delta));
					}
					group.createPath({}).
						moveTo(circle.cx, circle.cy).
						lineTo(x1, y1).
					...
					if(fill && fill.type === "radial"){
						...
						if(this.opt.radGrad === "linear"){
							fill = this._pseudoRadialFill(fill, {x: circle.cx, y: circle.cy}, r, start, end);
						}
					}else if(fill && fill.type === "linear"){
						fill = this._plotFill(fill, dim, offsets);
						fill = this._shapeFill(fill, shape.getBoundingBox());
					}
					shape.setFill(fill);
				}
				this.dyn.push({color: color, fill: fill, stroke: stroke});
